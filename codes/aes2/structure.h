...
void KeyExpansionCore(unsigned char * in, unsigned char i) {
	...
	unsigned char t = in[0];     // 1 byte circular left shift, rotword
	in[0] = in[1];
	in[1] = in[2];
	in[2] = in[3];
	in[3] = t;

	// S-box 4 bytes     subword
	in[0] = s[in[0]];
	in[1] = s[in[1]];
	in[2] = s[in[2]];
	in[3] = s[in[3]];

	// RCon     xor the leftmost byte  the first byte will be changed
	in[0] ^= rcon[i];
}
...
void KeyExpansion(unsigned char inputKey[16], unsigned char expandedKeys[176]) {
	...
	int rconIteration = 1;
	unsigned char tmpCore[4];    // a word

	while (bytesGenerated < 176)    // at a time a word
    {
		...
		if (bytesGenerated % 16 == 0)     // complex g function
        {
